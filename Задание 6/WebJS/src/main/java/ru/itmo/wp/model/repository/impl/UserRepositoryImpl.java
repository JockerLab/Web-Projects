...
public class UserRepositoryImpl implements UserRepository {
    ...
    public User findByLoginOrEmailAndPasswordSha(String loginOrEmail, String passwordSha) {
        String sql = "SELECT * FROM User WHERE (login=? OR email=?) AND passwordSha=?";
    ...
    public void save(User user, String passwordSha) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("INSERT INTO `User` (`login`, `passwordSha`, `email`, `creationTime`, `admin`) VALUES (?, ?, ?, NOW(), ?)", Statement.RETURN_GENERATED_KEYS)) {
    ...
    public void setAdmin(long id, boolean value) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE User SET admin=? WHERE id=?")) {
                ...
                if (statement.executeUpdate() != 1) {
                    throw new RepositoryException("Can't change `admin` in User.");
                }
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't find User.", e);
