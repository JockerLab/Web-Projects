...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            ...
            for (Method currentMethod : pageClass.getDeclaredMethods()) {
                if (currentMethod.getName().equals(route.getAction())) {
        ...
        try {
            ...
            List<Object> parametersArray = new ArrayList<>();
            for (Object parameter : parameterTypeList) {
                if (Map.class.equals(parameter)) {
                    parametersArray.add(view);
                }
                if (HttpServletRequest.class.equals(parameter)) {
                    parametersArray.add(request);
                }
            }
            Object[] array = parametersArray.toArray();
