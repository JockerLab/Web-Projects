...
class HttpServer {
    ...
    private String getHash(String file) throws IOException, NoSuchAlgorithmException {
        file = "static" + file;
    ...
    private void process(Request request, Response response) throws IOException, NoSuchAlgorithmException {
        ...
        if (file.isFile()) {
            ...
            String hash = getHash(request.getUri());
            ...
            if (file.isDirectory()) {
                byte[] body = (new String("static_index")).getBytes();
                response.setStatusCode(200);
                response.setBody(body);
                response.setHeader("Content-Length", Integer.toString(body.length));
                response.setHeader("Content-Type", new String("text/html"));
            } else {
    ...
    private String getContentType(File file) {
        ...
        if (path.endsWith(".html") || path.endsWith(".htm") || path.length() == 0) {
            return "text/html";
        }
    ...
    private static final class Request {
        ...
        private Request(String request) throws UnsupportedEncodingException {
            ...
            if (firstLineTokens[1].equals("/")) {
                ...
                uri = URLDecoder.decode(firstLineTokens[1], "UTF-8");
            }
            headers = new HashMap<>();
